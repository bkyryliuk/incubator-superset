...
class CsvToDatabaseView(SimpleFormView):
    ...
    def form_post(self, form):
        ...
        uploaded_csv_schema = config["UPLOADED_CSV_SCHEMA_FUNC"](
            database, g.user, schema_name
        )
        if uploaded_csv_schema:
            if "." in table_name or (
                schema_name and schema_name != uploaded_csv_schema
            ):
                message = _(
                    "You can't specify a namespace. All tables will be "
                    'uploaded to the "%(uploaded_csv_schema)s" namespace.',
                    uploaded_csv_schema=uploaded_csv_schema,
                )
                flash(message, "danger")
                return redirect("/csvtodatabaseview/form")
            schema_name = uploaded_csv_schema
