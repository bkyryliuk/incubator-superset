...
class CoreTests(SupersetTestCase):
    ...
    @mock.patch(
        ...
        {**app.config, "UPLOADED_CSV_SCHEMA_FUNC": lambda d, u, s: "admin_database"},
    )
    def test_import_csv_enforced_schema(self):
        ...
        try:
            ...
            success_msg = f'CSV file "{filename}" uploaded to table "{table_name}"'
            ...
            # no user specified schema, replace
            resp = self.upload_csv(
                filename,
                table_name,
            ...
            self.assertIn(
                'You can\'t specify a namespace. All tables will be uploaded to the "admin_database" namespace',
                resp,
            )
